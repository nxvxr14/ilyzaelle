...
import { getAIDash } from "@/api/ProjectApi";
...
function DashboardZoneView({ gVarData, projectId, serverAPIKey }: DashboardZoneViewProps) {
    ...
    // Query to get AIDashCode for the "AI Dash" button
    const { data: aiDashData } = useQuery({
        queryKey: ['aidash', projectIdFromParams],
        queryFn: () => getAIDash(projectIdFromParams),
        enabled: !!projectIdFromParams,
    });
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden p-6 mb-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
                    ...
                    <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-3">
                        <button
                            className="bg-black text-white hover:bg-[#FFFF44] hover:text-black font-bold px-6 py-3 text-sm sm:text-lg cursor-pointer transition-all rounded-xl shadow-sm hover:shadow-md flex items-center justify-center gap-2"
                        ...
                        <button
                            className="bg-purple-600 text-white hover:bg-purple-700 font-bold px-6 py-3 text-sm sm:text-lg cursor-pointer transition-all rounded-xl shadow-sm hover:shadow-md flex items-center justify-center gap-2"
                            onClick={() => navigate(`/projects/${projectIdFromParams}/ai-dashboard`)}
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <button
                            className={`font-bold px-6 py-3 text-sm sm:text-lg transition-all rounded-xl shadow-sm flex items-center justify-center gap-2 ${
                                aiDashCode
                        ...
                        <ClearDashboardButton onClear={clearAllComponents} className="border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 font-bold px-6 py-3 text-sm sm:text-lg cursor-pointer transition-all rounded-xl shadow-sm hover:shadow-md flex items-center justify-center gap-2" />
                    </div>
                </div>
